...
export const TRACE_LOGGING_GUIDE = [
  ...
  "- EVERY agent (and tool) message MUST carry its own `intent_confidence` and `resolution_confidence` (0-1): the confidence that informed that specific reply. The console renders them as pie charts under each turn so staff can see the confidence trajectory across the conversation.",
  "- Use `metadata` on a message for anything structured you want staff to see (policy id, rule that fired).",
